...
string rep_with_upper(string password) {
    char char_to_replace = password[rand() % password.size()];
    string new_password = "";
    for (char &c: password) {
        if (c == char_to_replace) {
            new_password += toupper(c);
        } else {
            new_password += c;
        }
    }
...
string swap_letters(string str) {
    if (str.size() < 2) {
        ...
        return str.substr(str.size() - 2) + str.substr(2, str.size() - 4) + str.substr(0, 2);
    }
}
...
int main() {
    while (true) {
        int user_number;
        cout << "Enter the number of words in your password between 3 and 7: ";
        cin >> user_number;
        cout<<endl;
